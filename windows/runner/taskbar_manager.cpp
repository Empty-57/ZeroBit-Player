...
TaskbarManager::~TaskbarManager() {
    ...
    if (icon_next_) DestroyIcon(icon_next_);
    if (taskbar_list_) taskbar_list_->Release();
...
void TaskbarManager::SetCustomThumbnail(const std::vector<uint8_t>& imageBytes) {
    if (imageBytes.empty() || !hwnd_) return;
...
void TaskbarManager::ResetThumbnail() {
    ...
    if (custom_thumbnail_bitmap_) {
        DeleteObject(custom_thumbnail_bitmap_);
        custom_thumbnail_bitmap_ = nullptr;
    }
}
...
std::optional<LRESULT> TaskbarManager::HandleWindowMessage(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
    ...
    switch (message) {
        case WM_COMMAND: {
            ...
            if (wmEvent == THBN_CLICKED) { // 任务栏缩略图按钮点击事件
                std::string action;
                if (wmId == IDM_THUMBBUTTON_PREV) action = "prev";
                else if (wmId == IDM_THUMBBUTTON_TOGGLE) action = "toggle";
                else if (wmId == IDM_THUMBBUTTON_NEXT) action = "next";
