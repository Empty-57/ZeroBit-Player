...
final Uint8List _qrcKey = Uint8List.fromList(
...
/// 从 a（长度至少8的 Uint8List）中取第 b 位，左移 c 位后返回
int _bitnum(Uint8List a, int b, int c) {
  final pos = (b ~/ 32) * 4 + 3 - ((b % 32) ~/ 8);
  return ((a[pos] >>> (7 - b % 8)) & 1) << c;
}

/// 从 32 位整数 a 中无符号右移第 b 位，左移 c 位后返回
int _bitnumIntr(int a, int b, int c) {
  return ((a >>> (31 - b)) & 1) << c;
}

/// 将 32 位整数 a 左移 b 位，并与 0x80000000 做与后，逻辑右移 c 位
int _bitnumIntl(int a, int b, int c) {
  return ((a << b) & 0x80000000) >>> c;
}

/// 将 6 位输入 a 转换为对应 S-Box 索引
int _sboxBit(int a) {
  return (a & 32) | ((a & 31) >>> 1) | ((a & 1) << 4);
}

/// 初始置换，将输入 8 字节（64 位）拆成两个 32 位数 [s0, s1]
List<int> _initialPermutation(Uint8List inputData) {
  var s0 = 0, s1 = 0;

  // s0 calculation
  s0 |= _bitnum(inputData, 57, 31);
  s0 |= _bitnum(inputData, 49, 30);
  s0 |= _bitnum(inputData, 41, 29);
  s0 |= _bitnum(inputData, 33, 28);
  s0 |= _bitnum(inputData, 25, 27);
  s0 |= _bitnum(inputData, 17, 26);
  s0 |= _bitnum(inputData, 9, 25);
  s0 |= _bitnum(inputData, 1, 24);
  s0 |= _bitnum(inputData, 59, 23);
  s0 |= _bitnum(inputData, 51, 22);
  s0 |= _bitnum(inputData, 43, 21);
  s0 |= _bitnum(inputData, 35, 20);
  s0 |= _bitnum(inputData, 27, 19);
  s0 |= _bitnum(inputData, 19, 18);
  s0 |= _bitnum(inputData, 11, 17);
  s0 |= _bitnum(inputData, 3, 16);
  s0 |= _bitnum(inputData, 61, 15);
  s0 |= _bitnum(inputData, 53, 14);
  s0 |= _bitnum(inputData, 45, 13);
  s0 |= _bitnum(inputData, 37, 12);
  s0 |= _bitnum(inputData, 29, 11);
  s0 |= _bitnum(inputData, 21, 10);
  s0 |= _bitnum(inputData, 13, 9);
  s0 |= _bitnum(inputData, 5, 8);
  s0 |= _bitnum(inputData, 63, 7);
  s0 |= _bitnum(inputData, 55, 6);
  s0 |= _bitnum(inputData, 47, 5);
  s0 |= _bitnum(inputData, 39, 4);
  s0 |= _bitnum(inputData, 31, 3);
  s0 |= _bitnum(inputData, 23, 2);
  s0 |= _bitnum(inputData, 15, 1);
  s0 |= _bitnum(inputData, 7, 0);

  // s1 calculation
  s1 |= _bitnum(inputData, 56, 31);
  s1 |= _bitnum(inputData, 48, 30);
  s1 |= _bitnum(inputData, 40, 29);
  s1 |= _bitnum(inputData, 32, 28);
  s1 |= _bitnum(inputData, 24, 27);
  s1 |= _bitnum(inputData, 16, 26);
  s1 |= _bitnum(inputData, 8, 25);
  s1 |= _bitnum(inputData, 0, 24);
  s1 |= _bitnum(inputData, 58, 23);
  s1 |= _bitnum(inputData, 50, 22);
  s1 |= _bitnum(inputData, 42, 21);
  s1 |= _bitnum(inputData, 34, 20);
  s1 |= _bitnum(inputData, 26, 19);
  s1 |= _bitnum(inputData, 18, 18);
  s1 |= _bitnum(inputData, 10, 17);
  s1 |= _bitnum(inputData, 2, 16);
  s1 |= _bitnum(inputData, 60, 15);
  s1 |= _bitnum(inputData, 52, 14);
  s1 |= _bitnum(inputData, 44, 13);
  s1 |= _bitnum(inputData, 36, 12);
  s1 |= _bitnum(inputData, 28, 11);
  s1 |= _bitnum(inputData, 20, 10);
  s1 |= _bitnum(inputData, 12, 9);
  s1 |= _bitnum(inputData, 4, 8);
  s1 |= _bitnum(inputData, 62, 7);
  s1 |= _bitnum(inputData, 54, 6);
  s1 |= _bitnum(inputData, 46, 5);
  s1 |= _bitnum(inputData, 38, 4);
  s1 |= _bitnum(inputData, 30, 3);
  s1 |= _bitnum(inputData, 22, 2);
  s1 |= _bitnum(inputData, 14, 1);
  s1 |= _bitnum(inputData, 6, 0);

  return [s0 >>> 0, s1 >>> 0]; // 转换为无符号32位整数
}

/// 逆置换，将两个 32 位数 s0, s1 合成 8 字节数据
...
/// 基本函数 f，对 32 位 state 和 6 字节子密钥进行运算，返回 32 位结果
int _f(int state, List<int> key) {
  int uState = state & 0xFFFFFFFF;

  int t1 =
      (_bitnumIntl(uState, 31, 0) |
      ((uState & 0xF0000000) >>> 1) |
...
List<List<int>> _keySchedule(Uint8List key, int mode) {
  final List<List<int>> schedule = List.generate(16, (_) => List.filled(6, 0));
...
/// 3DES 密钥调度：返回包含 3 个 16 轮子密钥表的列表
...
/// 3DES 分组加/解密：对单个 8 字节块，依次经过三个 DES
...
Future<String?> qrcDecrypt({
  required dynamic encryptedQrc,
  ...
  if (encryptedQrc is String) {
    encryptedBytes = Uint8List.fromList(hex.decode(encryptedQrc));
